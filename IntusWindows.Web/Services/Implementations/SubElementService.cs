...
namespace IntusWindows.Web.Services.Implementations
{
    public class SubElementService : ISubElementService
    {
        ...
        public async Task<bool> DeleteSubElement(SubElementDTO subElementDTO)
        {
            try
            {
                var response = await httpClient.DeleteAsync($"api/SubElements/{subElementDTO.ID}");
        ...
        public async Task<IEnumerable<SubElementDTO>> GetSubElements(int windowId)
        {
            try
            {
